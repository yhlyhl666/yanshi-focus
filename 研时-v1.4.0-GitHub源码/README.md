...
> 一款为考研复习设计的 Windows 桌面工具。把专注计时、学习任务、精力负荷、真实放松和妙脆角猫陪伴放在同一个安静的学习空间里；每次减压都有记录，误操作也可以撤销。
...
所有任务、专注记录、减压记录、设置和自定义放松选项都优先保存在本机，不需要注册账号。
...
今日减压记录会列出活动、完成时间、时长和减压值，并直接展示“任务负荷 − 已抵扣减压 = 当前压力”。误记后可逐条撤销，并在 10 秒内恢复。

![今日减压记录、压力计算与撤销入口](docs/images/recovery-history.png)
